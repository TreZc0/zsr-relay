...
leaderboard.on('change', newVal =>
{
    ...
    if (newVal.forceTime > 0)
        changeTime = true;

    if (changeTime)
    {
        if (newVal.forceTime > 59)
        {
			minutes = Math.floor(newVal.forceTime / 60);
            seconds = newVal.forceTime - (minutes * 60);
		...
		nodecg.log.info("Adjust time to: " + newTimeString + " ; Raw Time: " + newVal.forceTime);
    }

    if (newVal.forceStart)
    {
        newVal.forceStart = false;

		TimeObject.setDelay(stopwatch.value, newVal.forceDelay);
		newVal.forceDelay = 0;

		if (changeTime)
			editTime({ index: "master", newTime: newTimeString })
		else
...
function reset() {
	stop();
	TimeObject.setDelay(stopwatch.value, 0);
	TimeObject.setSeconds(stopwatch.value, 0);

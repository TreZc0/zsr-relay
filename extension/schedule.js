...
nodecg.listenFor('suspendRunnerUpdates', _suspendRunnerUpdates);

function _suspendRunnerUpdates()
{
    runnerUpdateSuspended = true;
}
...
function update() {
	...
	const runnersPromise = request({
		uri: nodecg.bundleConfig.useMockData ?
			'https://dl.dropboxusercontent.com/s/rvdsbfmbfagugge/runners.json' :
	...
	*/
	...
	const adsPromise = request({
		uri: nodecg.bundleConfig.useMockData ?
			'https://dl.dropboxusercontent.com/s/waa6usfw2rpreia/ads.json' :
			'https://private.gamesdonequick.com/tracker/gdq/ads/1/',
		json: true
	});

	const interviewsPromise = request({
		uri: nodecg.bundleConfig.useMockData ?
			'https://dl.dropboxusercontent.com/s/7eyyx51i83fej0b/interviews.json' :
			'https://private.gamesdonequick.com/tracker/gdq/interviews/1/',
		json: true
	});

	return Promise.all([
		runsPromise, adsPromise, interviewsPromise //runnersPromise, 
	]).then(([runsJSON, adsJSON, interviewsJSON]) =>  //runnersJSON, 
	{
		/*
		...
		if (!deepEqual(formattedRunners, runnersRep.value) && !runnerUpdateSuspended) 
		{
		    runnersRep.value = clone(formattedRunners);
		}
		else
		{
		    formattedRunners = clone(runnersRep.value);
		}
		*/
...
function formatRun(run, formattedRunners) {
	const runners = run.fields.runners.slice(0, 4).map(runnerId => { //adjust for more runners
		return {
	...
	return {
		...
		shortName: run.fields.shortname || 'Unknown',

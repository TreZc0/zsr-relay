(function () {
	 ...
	 * @polymer
	 */
	class ZsrOmnibarBidwars extends Polymer.Element {
		static get is() {
			return 'zsr-omnibar-bidwars';
		}
		...
		enter(displayDuration, scrollHoldDuration) {
			...
			this.bidWars.forEach((bidWar, index) => {
				...
				const bidElements = bidWar.options.slice(0, MAX_OPTIONS).map((option, index) => {
					const element = document.createElement('zsr-omnibar-bidwar-option');
					element.bid = option;
				...
				if (bidElements.length <= 0) {
					const placeholder = document.createElement('zsr-omnibar-bidwar-option');
					placeholder.bid = {};
				...
				if (index === 0) {
					tl.add(this.$.label.enter(bidWar.description +"<br>"+ bidWar.speedrun));
				} else {
					tl.add(this.$.label.change(bidWar.description +"<br>"+ bidWar.speedrun));
				}
	...
	customElements.define(ZsrOmnibarBidwars.is, ZsrOmnibarBidwars);
})();

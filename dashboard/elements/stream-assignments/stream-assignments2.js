(function () {
    ...
    class StreamAssignments2 extends Polymer.MutableData(Polymer.Element) {
        static get is() {
            return 'stream-assignments2';
        }
                ...
                replicant.once('change', () => {
                    ...
                    if (numDeclared >= replicants.length) {
                        twitchPlayer.on('change', newVal => {
                            ...
                            this.runnerList = newVal.team2;

                            var count = 0;
                            var link = "kadgar.net Link";

                            newVal.team2.find(runner => {
                                if (runner.checked == true && runner.id == "---> Team B") {
                                    if (count == 0) {
        ...
        _assignStreams()
        {
			nodecg.sendMessage("askSwitchRunners", 2);
        ...
        _checkboxChanged(e)
        {
            ...
            twitchPlayer.value.team2.forEach(runner => {
                
                if (runner.checked == true)
                    count++;
            });

            twitchPlayer.value.team2.find(runner => {
                if (runner.name === name)
				{
                    ...
                    if (target.checked)
                    {
                        ...
                        twitchPlayer.value.team2.find(runnerSearch => {
                            if (runnerSearch.checked == true && runnerSearch.id == "---> Team B")
                            {
                                runnerFound = true;
    ...
    customElements.define(StreamAssignments2.is, StreamAssignments2);
})();
